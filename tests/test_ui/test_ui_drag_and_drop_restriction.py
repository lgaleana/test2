...
def test_ui_drag_and_drop_restriction(browser):
    ...
    for headline in page.query_selector_all("#image-result p.draggable"):
        ...
        print(f"Initial position: x={box['x']}, y={box['y']}, width={box['width']}, height={box['height']}")  # Log initial position
        ...
        print(f"New position: x={new_box['x']}, y={new_box['y']}, width={new_box['width']}, height={new_box['height']}")  # Log new position
        ...
        print(f"Container position: left={container_box['left']}, top={container_box['top']}, width={container_box['width']}, height={container_box['height']}")  # Log container position

        # Log the restriction check
        print(f"Restriction check: {container_box['left']} <= {new_box['x']} <= {container_box['left'] + container_box['width'] - new_box['width']}")
        print(f"Restriction check: {container_box['top']} <= {new_box['y']} <= {container_box['top'] + container_box['height'] - new_box['height']}")
